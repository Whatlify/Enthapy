...
public static class Reach
{
	...
	{
		Memory gClass = new Memory(MainForm.uint_0);
        StandardRng rng = StandardRng.Create();
        Bernoulli Chancer = Bernoulli.FromRatio((uint)MainForm.MainF.sldChance.Int32_0, 100);
        bool Chance = Chancer.Sample(rng);
        while (true)
		{
			double num = new Random().Next(MainForm.MainF.sldMinimumReach.Int32_0, MainForm.MainF.sldMaximumReach.Int32_0) / 100.0;
				...
				foreach (long item in list_2.ToList())
				{
					foreach (long item2 in list_1.ToList())
					{
						...
						if (num3 <= 7.5 && num3 >= 4.5)
						{
							...
							{
								if ((MainForm.MainF.cbReach.Checked && !MainForm.MainF.cbMoving.Checked) && Chance || (MainForm.MainF.cbReach.Checked && MovingCheck()))
								{
